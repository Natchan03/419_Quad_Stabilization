...
def getLastCheckpoint(load_dir):
    with open(os.path.join(load_dir, 'checkpoint'), 'r') as f:
        line = f.readline()
        start = line.find('\"') + 1
        end = line.rfind('\"')
        path = line[start:end]
...
def run(env, algorithm, args, params=None, load=False, loadpath=None, loaditer=None, save_obs=False):
    ...
    # load trained policy
    if load and loadpath is not None:
        pi = init_policy
        # pi.load_model(loadpath, iteration=loaditer)
        pi.load_model_from_path(getLastCheckpoint(loadpath))
        if loaditer is None:
            loaditer = 0
        pi.save_model(args['MODEL_DIR'], iteration=loaditer)
...
if __name__ == "__main__":
    with tf.device('/gpu:1'):
        ...
        if args['algo'] == "ppo":
            RUN_DIR = os.path.join(os.getcwd(), 'runs', 'ppo_ttr')
        ...
        if args['algo'] == "ppo":
            ...
            env = gym.make(args['gym_env'], rew=args['rew_type'])
            LOAD_DIR = curFolder + '/runs/ppo_ttr/model'
